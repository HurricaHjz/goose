...
def parse_literal(alist, type_dict, predicate_dict, negated=False): # return either Atom or Negated Atom e.g. clear ?b1 will be Atom with predicate="clear", args ="?b1"
    ...
    if arity != len(alist) - 1:
        raise SystemExit(
            "predicate used with wrong arity: (%s)" % " ".join(alist)
        )
...
def parse_domain_pddl(domain_pddl):
    ...
    for opt in iterator:
        ...
        if seen_fields and correct_order.index(
            seen_fields[-1]
        ) > correct_order.index(field):
            msg = (
                "\nWarning: %s specification not allowed here (cf. PDDL BNF)"
                % field
            )
            print(msg, file=sys.stderr)
...
def check_atom_consistency(
    ...
    if atom in same_truth_value:
        if not atom_is_true:
            atom = atom.negate()
        print(
            "Warning: %s is specified twice in initial state specification"
            % atom
        )
